...
struct Cta
{
	...
	struct Iterate
	{
			...
			const SizeT 	&guarded_elements)
		{
			...
			// Iterate next element
			Iterate<COUNT + 1, MAX>::ScatterGlobal(items, ranks, digit_offsets, d_out, guarded_elements);
		}
			...
			const SizeT 	&valid_elements)
		{
			...
			{
				int my_exclusive_scan 	= smem_storage.ranking_storage.warpscan[0][(WARP_THREADS / 2) + my_digit - 1];
				...
				while (my_aligned_offset < my_digit_count)
				{
					if ((my_aligned_offset >= 0) && (my_exclusive_scan + my_aligned_offset < valid_elements))
					{
						...
						if (PADDED_EXCHANGE) gather_offset = util::SHR_ADD(gather_offset, LOG_MEM_BANKS, gather_offset);

						T datum = buffer[my_exclusive_scan + my_aligned_offset];
		...
		const SizeT 	&guarded_elements)
	{
		if ((LOAD_MODIFIER == util::io::ld::tex) && (guarded_elements >= TILE_ELEMENTS))
		{
			...
			KeyTexType *vectors = (KeyTexType *) converted_keys;
		...
		const SizeT &guarded_elements = TILE_ELEMENTS)
	{
		...
		GatherScatterValues(values, digit_offsets, ranks, tex_offset, guarded_elements);
	}
